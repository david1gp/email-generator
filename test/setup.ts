...
process.env.MARKDOWN_RENDER_TOKEN = "dev-only-markdown-token"
...
beforeAll(async () => {
  ...
  if (await isServerRunning()) {
    console.log(`Server already running at ${BASE_URL} (health check passed) → skipping start`)
    process.env.EMAIL_GENERATOR_URL = BASE_URL
    return
  }
