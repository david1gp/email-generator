...
import { t4signIn } from "./t4signIn.js"

export function SignInV1Template(p: SignInV1Type) {
  ...
  const title = p.subject ?? t1(tt.Your_Sign_in_code_x, p.code)
  const codeLabel = p.codeLabel ?? t0(tt.Copy_and_paste_this_signin_code)
  const magicLinkText = p.magicLinkText ?? t0(tbOrUseTheMagicLinkBelow)
  const buttonText = p.buttonText ?? t0(tt.Sign_in_link)
