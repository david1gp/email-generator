...
export function SignInV1Template(p: SignInV1Type) {
  ...
  const title = t1(tt.Your_Sign_in_code_x, p.code)
  ...
  return (
    ...
    >
      ...
      <Section className={""}>
        <Text className={sectionTextClass}>{t0(tt.Copy_and_paste_this_signin_code)}</Text>
        <CodeBlock className={"px-2"} text={p.code} />
      </Section>

      <Section className={sectionClass}>
        <Text className={sectionTextClass}>{t0(tbOrUseTheMagicLinkBelow)} </Text>
        <LinkButton url={p.url} text={t0(tt.Sign_in_link)} />
      </Section>
